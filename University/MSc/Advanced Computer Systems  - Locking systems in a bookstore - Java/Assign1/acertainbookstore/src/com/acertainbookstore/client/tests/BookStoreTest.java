...
public class BookStoreTest {
	...
	/**
	 * Positive RateBooks test with a simple verification if the rate is being added
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	...
	public void testRateBooks() throws BookStoreException {
		...
		List<StockBook> booksInStorePostTest = storeManager.getBooks();
		// Check pre and post state are same.
		for(int x =0; x < booksInStorePreTest.size();x++){
			
			if(booksInStorePreTest.get(x).getISBN() == TEST_ISBN){
				assertTrue((booksInStorePreTest.get(x).getNumTimesRated()+1) == (booksInStorePostTest.get(x).getNumTimesRated()));
			}
		}
	}
	/**
	...
	public void testRateNonExistingISBN() throws BookStoreException {
		...
		HashSet<BookRating> booksToRate = new HashSet<BookRating>();
		booksToRate.add(new BookRating(TEST_ISBN, 5)); // valid
		booksToRate.add(new BookRating(100000, 10)); // invalid
		...
		// Check pre and post state are same.
		for(int x =0; x < booksInStorePreTest.size();x++){
			assertTrue(booksInStorePreTest.get(x).getNumTimesRated() == booksInStorePostTest.get(x).getNumTimesRated());
		}
	}
	
	/**
	 * Tests that you can't Rate negative in a book.
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	@Test
	public void testRateNegativeBook() throws BookStoreException {
		List<StockBook> booksInStorePreTest = storeManager.getBooks();

		// Try to buy a book with ISBN which does not exist.
		HashSet<BookRating> booksToRate = new HashSet<BookRating>();
		booksToRate.add(new BookRating(TEST_ISBN, -5)); // invalid
		try {
			client.rateBooks(booksToRate);
			fail();
		} catch (BookStoreException ex) {
	 ...
	 */
	@Test
	public void testNullRateBook() throws BookStoreException {
		List<StockBook> booksInStorePreTest = storeManager.getBooks();

		// Try to buy a book with ISBN which does not exist.
		HashSet<BookRating> booksToRate = null;
		try {
			client.rateBooks(booksToRate);
			fail();
		} catch (BookStoreException ex) {
	...
	/**
	 * Positive Tests on Top Rated Books
	 *
	 * @throws BookStoreException
	 *             the book store exception
	 */
	...
	public void testTopRatedBooks() throws BookStoreException {
		...
		addBooks(TEST_ISBN+2,1);
		addBooks(TEST_ISBN+3,1);
		

		// Try to buy a book with ISBN which does not exist.
		HashSet<BookRating> booksToRate = new HashSet<BookRating>();
		booksToRate.add(new BookRating(TEST_ISBN, 1)); // valid
		booksToRate.add(new BookRating(TEST_ISBN+1, 3));
		booksToRate.add(new BookRating(TEST_ISBN+2, 4));
		...
		List<Book> testList = client.getTopRatedBooks(2);
		
		// Check pre and post state are same.
					assertEquals(testList.get(0).getISBN(),(TEST_ISBN+3) );
					assertEquals(testList.get(1).getISBN(),(TEST_ISBN+2) );
